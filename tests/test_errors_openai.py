...
def test_too_many_items() -> None:
    with pytest.raises(Exception):
        validate_text_limits(["a", "b", "c"], max_batch_size=2, max_chars_per_item=10, max_total_chars=10)
...
def test_too_many_total_chars() -> None:
    with pytest.raises(Exception):
        validate_text_limits(["abcd", "efgh"], max_batch_size=3, max_chars_per_item=10, max_total_chars=7)
