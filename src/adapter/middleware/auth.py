...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:  # type: ignore[override]
        ...
        if self.settings.auth_mode == "bearer":
            ...
            if auth != expected:
                return JSONResponse(status_code=401, content=openai_error_dict(401, "invalid bearer token"))
            return await call_next(request)

        if self.settings.auth_mode == "basic":
            if not auth.startswith("Basic "):
                return JSONResponse(status_code=401, content=openai_error_dict(401, "basic auth required"))
            encoded = auth.split(" ", 1)[1]
            try:
                decoded = base64.b64decode(encoded).decode("utf-8")
            except Exception:
                return JSONResponse(status_code=401, content=openai_error_dict(401, "invalid basic token"))
